...
class SighthingsCountAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        # Conteo total de registros
        count_register = Sighting.objects.count()

        # Fecha de ayer
        yesterday = now().date() - timedelta(days=1)

        # Contar registros creados ayer
        count_yesterday = Sighting.objects.filter(date=yesterday).count()
        ...
        return Response(
            {
                ...
                "total_sighthings": count_register,
